...
const SEED_CONTENT_VERSION = "2026-06-17-bpd-v1";
...
// The eight Psychodynamic Treatment of BPD sections become the course topics.
...
const HOMEWORK_INSTRUCTIONS =
  ...
  "name a specific hidden function or conflict beneath the symptom or behavior in BPD and the cheap observation that would disconfirm it. The cautious " +
...
// The course now uses the eight Psychodynamic Treatment of BPD sections for content and a
...
export async function seedIfEmpty(): Promise<void> {
  // The course was migrated to the Psychodynamic Treatment of BPD 101 syllabus. Detect the marker topic;
  ...
  await db.transaction(async (tx) => {
    ...
    if ((row.n ?? 0) > 0) {
      logger.warn(
        "Seed: stale course content detected — replacing with the Psychodynamic Treatment of BPD 101 curriculum",
      );
      await tx.execute(
