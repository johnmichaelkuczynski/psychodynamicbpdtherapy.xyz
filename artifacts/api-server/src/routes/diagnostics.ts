    ...
    steps.push(
      ...
      await run(`Practice problem ${i + 1}: generate + grade + adapt`, async () => {
        ...
        const gen = await chatJson<{
          ...
          `You generate a single introductory psychodynamic treatment of BPD (a depth-oriented approach to the meaning beneath the instability of emotion, identity, and relationships in borderline personality disorder) practice problem on "${topic.title}" at easy difficulty, with a short answer (a word, term, "yes"/"no", or short phrase). Respond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}.`,
      ...
      await run(
        ...
        async () => {
          ...
          const derived = await chatText(
            "You are a strong introductory psychodynamic treatment of BPD student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for an introductory psychodynamic treatment of BPD course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are an introductory psychodynamic treatment of BPD lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
