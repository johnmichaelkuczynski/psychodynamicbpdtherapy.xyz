    ...
    steps.push(
      ...
      await run(`Practice problem ${i + 1}: generate + grade + adapt`, async () => {
        ...
        const gen = await chatJson<{
          ...
          `You generate a single introductory psychodynamic therapy (a depth-oriented approach to the mind beneath the surface) practice problem on "${topic.title}" at easy difficulty, with a short answer (a word, term, "yes"/"no", or short phrase). Respond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}.`,
      ...
      await run(
        ...
        async () => {
          ...
          const derived = await chatText(
            "You are a strong introductory psychodynamic therapy student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for an introductory psychodynamic therapy course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are an introductory psychodynamic therapy lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
