...
router.post("/practice/sessions/:sessionId/next", async (req, res): Promise<void> => {
  ...
  try {
    generated = await chatJson<{
      ...
      `You generate a single introductory psychodynamic treatment of OCD (reading the meaning beneath obsessions and compulsions) practice problem for a curious beginner. The problem MUST be on the topic "${topic.title}" and at difficulty "${difficultyLabel}" (${difficulty.toFixed(
        1,
      )}/5). Test general knowledge of the SUBJECT of the psychodynamic treatment of OCD (reading the meaning beneath obsessions and compulsions), not recall of any particular book, lecture, or course. The question MUST be fully self-contained and answerable by anyone who knows the subject: do NOT reference "the lecture", "the text", "the course", "the example", "the case", or any named character or example a student would only recognize from a specific reading.\n\nSTRICT QUESTION RULES (no exceptions):\n- Every question must present a SPECIFIC, concrete clinical or everyday-relationship scenario (e.g. someone who rechecks a locked door until a specific dread lifts, a person who washes only after touching things tied to one particular fear, a patient whose counting ritual appears right when an angry thought does) and ask the student to APPLY the topic's idea to THAT scenario — to infer the hidden function or conflict, judge, compare, explain, interpret, or decide, and name an observation that would disconfirm the reading.\n- NEVER ask for a definition, a term, or what something is "called"; NEVER ask the student to recite an abstract formulation from any reading.\n- NEVER write a question whose answer is a single word, a single term, or a bare "yes"/"no". The answer must require reasoning that is HARD TO SHARE: the student must explain their thinking about the specific scenario in 2-4 sentences.\n- The "correctAnswer" is the model answer (2-4 sentences) showing the operational reasoning a strong student would give for THIS scenario. The "explanation" briefly says what earns full credit.\n\nRespond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}. Avoid these recent prompts: ${JSON.stringify(
    ...
    generated = {
      prompt: `Practice (${topic.title}): A classmate described a specific situation involving an obsession or compulsion but isn't sure what to make of it. Using the idea behind "${topic.title}", walk through how you'd help them make sense of it, and explain your reasoning in 2-4 sentences about that situation.`,
      correctAnswer:
    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise introductory psychodynamic treatment of OCD tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
