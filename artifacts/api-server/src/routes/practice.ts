...
router.post("/practice/sessions/:sessionId/next", async (req, res): Promise<void> => {
  ...
  try {
    generated = await chatJson<{
      ...
      `You generate a single introductory psychodynamic therapy (reading the mind beneath the surface) practice problem for a curious beginner. The problem MUST be on the topic "${topic.title}" and at difficulty "${difficultyLabel}" (${difficulty.toFixed(
        1,
      )}/5). Test general knowledge of the SUBJECT of psychodynamic therapy (reading the mind beneath the surface), not recall of any particular book, lecture, or course. The question MUST be fully self-contained and answerable by anyone who knows the subject: do NOT reference "the lecture", "the text", "the course", "the example", "the case", or any named character or example a student would only recognize from a specific reading.\n\nSTRICT QUESTION RULES (no exceptions):\n- Every question must present a SPECIFIC, concrete clinical or everyday-relationship scenario (e.g. someone whose anger at a boss is far bigger than the trigger, a person who keeps choosing the same kind of partner, a patient who calmly intellectualizes a painful loss) and ask the student to APPLY the topic's idea to THAT scenario — to infer the hidden dynamic, judge, compare, explain, interpret, or decide, and name an observation that would disconfirm the reading.\n- NEVER ask for a definition, a term, or what something is "called"; NEVER ask the student to recite an abstract formulation from any reading.\n- NEVER write a question whose answer is a single word, a single term, or a bare "yes"/"no". The answer must require reasoning that is HARD TO SHARE: the student must explain their thinking about the specific scenario in 2-4 sentences.\n- The "correctAnswer" is the model answer (2-4 sentences) showing the operational reasoning a strong student would give for THIS scenario. The "explanation" briefly says what earns full credit.\n\nRespond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}. Avoid these recent prompts: ${JSON.stringify(
    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise introductory psychodynamic therapy tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
