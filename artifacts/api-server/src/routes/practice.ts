    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise introductory cognitive science tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
